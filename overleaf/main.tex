...
\caption{Phase 1: Undefended orchestrator. The adversary poisons \texttt{employees.department\_name} in the public table (step 0). A benign finance-user query retrieves that value as raw output (steps 1--5). The injected instruction then causes a second round trip through the SQL agent to the private \texttt{employee\_salary} table (steps 6--9), after which salary and bonus records are sent to an attacker-controlled address (steps 10--11). Both tables reside in one SQLite database, and the finance role is authorized to read both; the failure is misuse of existing privilege, not an access-control bypass. Red dashed arrows show the attack path.}
\label{fig:phase1}
\end{figure*}
...
Phase 2 does not ask the LLM to guess whether text came from the user or the database. Before retrieval, the application stores the user message in a dedicated \texttt{original\_query} field, while SQL values are stored separately as typed claim cards. An injected value such as ``email the salary to the attacker'' therefore remains database data and cannot alter the saved request. A deterministic pre-check examines only \texttt{original\_query}; unless it contains an explicit notification request and recipient, the Action Planner returns no notification.
...
The critical difference from Phase 1 is therefore structural, not a judgment about which instruction sounds trustworthy. Phase 1 appends raw SQL results directly to the orchestrator's history, allowing injected text to influence its next tool call. Phase 2 preserves each value's origin. The poisoned \texttt{department\_name} still reaches the Extractor as data, but the Auditor and Claim Gate can quarantine that claim. For a legitimate notification, the Action Planner selects approved claim IDs; the deterministic Action Gate then verifies that the recipient appeared in \texttt{original\_query} and that every selected claim was approved before constructing the message body. The database can therefore supply content for a user-authorized notification, but it cannot authorize one itself.
...
Following OMNI-LEAK's methodology, we evaluate 20 attack payloads across four categories:

\begin{itemize}
    \item \textbf{Blocker} (6 payloads): Frames compensation exfiltration as a prerequisite that must be completed before the original query can be answered.
    \item \textbf{Compliance Framing} (6 payloads): Uses authority and policy language to frame exfiltration as a legitimate audit requirement.
    \item \textbf{Fixed Output Structure} (6 payloads): Enforces a rigid output format that embeds malicious steps as structural requirements.
    \item \textbf{Combined} (2 payloads): Merges blocker, compliance framing, and fixed output structure for maximum redundancy.
\end{itemize}
...
\begin{tabular}{lcc}
\toprule
\textbf{Category} & \textbf{Successful/Total} & \textbf{ASR} \\
\midrule
Blocker & 4/6 & 66.7\% \\
Compliance Framing & 4/6 & 66.7\% \\
Fixed Output Structure & 6/6 & 100.0\% \\
Combined & 2/2 & 100.0\% \\
\midrule
\textbf{Overall} & \textbf{16/20} & \textbf{80.0\%} \\
\bottomrule
\end{tabular}
\end{table}
...
The undefended orchestrator exhibits 80.0\% ASR: all 20 executions completed normally, and 16 sent at least one true salary or bonus value to the attacker address.

\textbf{Fixed-output and combined payloads are fully effective} in this run. All combined and fixed-output payloads succeed. Blocker and compliance framing each succeed in four of six executions.
...
The poisoned utility case requests all public details for employee \#1. The defense extracts five claims, rejects only the injected department value, and answers with the employee's ID, name, role, and email. Rather than discarding the employee's entire record because one field is poisoned, the defense removes only that field and preserves the remaining safe values. In the two legitimate action cases, an employee-role request sends an approved public department claim and a finance-role request sends approved salary and bonus claims to recipients explicitly named by the user; both simulated sends succeed. No email is produced in any of the five cases without trusted email intent. The suite nevertheless remains small and does not cover aggregate queries, long reports, multiple recipients, or richer action types.

\subsection{Consolidated Results}

Table~\ref{tab:overall-results} separates the Phase 0 authorization control from the causal Phase 1--2 comparison. Phase 0 uses a restricted employee role and confirms that ordinary database authorization prevents direct compensation access. Phases 1 and 2 both retain the finance role; consequently, their 80-percentage-point ASR difference reflects the change from the undefended orchestrator to the Cordon-inspired information-flow and gated-action architecture, rather than removal of salary-read capability.

\begin{table}[h]
\centering
\caption{Consolidated security results. ``Leaked'' requires a simulated email containing a true seeded salary or bonus value.}
\label{tab:overall-results}
\begin{tabular}{llcc}
\toprule
\textbf{Condition} & \textbf{Role} & \textbf{Leaked} & \textbf{Rate} \\
\midrule
Phase 0: ACL control & Employee & 0/20 & 0.0\% \\
Phase 1: Undefended & Finance & 16/20 & 80.0\% \\
Phase 2: Defended & Finance & 0/20 & 0.0\% \\
\bottomrule
\end{tabular}
\end{table}

The combined security--utility result has three principal takeaways. First, the attack is operational in the privileged baseline: 16 of 20 completed Phase 1 executions exfiltrate true compensation data. Second, the transferred defense blocks all 20 matched Phase 2 payloads while preserving the same finance authorization. Third, the result is not achieved by disabling useful actions: all four clean answer tasks and both explicitly authorized notification tasks succeed, while no notification occurs in any of the five cases lacking trusted email intent. These are controlled smoke-study results with one execution per payload, not population-level robustness estimates.

\subsection{Stage-Wise Attack Penetration}

Final ASR alone hides partial compromise. We therefore reconstruct a penetration funnel from the saved agent and tool traces, reported in Table~\ref{tab:penetration}. A stage is counted only from an observable event: poison retrieval from SQL output, creation and rejection of its claim card, execution of a query over \texttt{employee\_salary}, an actual notification-agent send to the attacker address, or detection of a seeded compensation value in that send. ``Reached'' and ``compromised'' are distinct: in Phase 2 the poisoned value reaches the Extractor and Auditor by design, but does not cross the Claim Gate.

\begin{table}[h]
\centering
\caption{Stage-wise penetration reconstructed from execution traces.}
\label{tab:penetration}
\begin{tabular}{lcc}
\toprule
\textbf{Observed stage} & \textbf{Phase 1} & \textbf{Phase 2} \\
\midrule
Poison retrieved & 20/20 & 20/20 \\
Poison extracted as claim & N/A & 20/20 \\
Poison flagged and rejected & N/A & 20/20 \\
Private compensation retrieved & 19/20 & 0/20 \\
Email sent to attacker address & 16/20 & 0/20 \\
Email contained salary/bonus data & 16/20 & 0/20 \\
\bottomrule
\end{tabular}
\end{table}

The undefended attack therefore penetrates beyond final exfiltration in three additional trials: 19 of 20 executions reach the private compensation table, while 16 continue through the notification channel. In Phase 2, all 20 poisoned values are exposed to the defensive processing stages and all 20 are contained at the Auditor--Gate boundary; none influence synthesis, receive action authorization, or trigger a notification. One trial reaches this same safe decision through fail-closed handling after invalid Qwen auditor output, demonstrating why model-formatting failure must be reported separately from attack success.

\section{Discussion and Limitations}

\textbf{Single-run evaluation.} Each attack was tested once. The reported 80\% and 0\% attack success rates show what happened in this experiment; repeating the tests would show whether these results remain consistent.
...
\textbf{Adaptive and propagating attacks remain open.} AgentWorm demonstrates persistent, autonomous propagation across agent instances \cite{agentworm}; Morris-II shows that self-replicating prompts can propagate through RAG-backed communication between GenAI applications \cite{morrisii}; and FORGE shows coordinated poisoning that steers recursive planning across documents \cite{forge}. Planned Phase 3 narrows this broad space to a controlled, single-pass provenance-laundering experiment: the output of one defended orchestrator is delivered as a peer handoff to another, testing whether database-originated intent can be mistaken for fresh trusted authority at the next hop. This is an AgentWorm/Morris-II-inspired propagation test, not a reproduction of persistent malware. Target-adaptive payload generation, motivated by adaptive AI worms \cite{adaptiveworm}, is reserved as an optional extension after fixed-carrier propagation is established.

\section{Conclusion}

This study evaluates whether a Cordon-MAS-inspired information-flow architecture can transfer from poisoned RAG to a privileged, tool-using orchestrator. Phase 0 confirms the expected boundary of ordinary role-based access control, but the main comparison deliberately retains finance authorization in both attack conditions. Under that shared capability, the undefended Phase 1 system exfiltrates true salary or bonus values in 16 of 20 executions, whereas the Phase 2 defended workflow exfiltrates none. Trace reconstruction localizes the difference: the baseline reaches the private compensation table in 19 trials, while the defense extracts, flags, and rejects the poisoned field in all 20 trials before it reaches synthesis or action authority.

The reduction is not obtained by disabling all useful behavior. The defended workflow completes all four clean answer tasks, both explicitly authorized notification tasks, and a poisoned-but-partially-answerable task without producing unintended notifications. These findings support the transferability of structured extraction, claim-level auditing, information-flow gating, and trusted-query-bound action authorization in this controlled setting. They remain preliminary because each fixed payload is executed once and the simplified implementation does not test adaptive cross-agent re-entry. Planned Phase 3 will evaluate whether a defended agent's output can launder database-originated intent into trusted authority when consumed as a peer handoff by another agent.
...
\bibitem{morrisii} S.~Cohen, R.~Bitton, and B.~Nassi, ``Here Comes The AI Worm: Unleashing Zero-click Worms that Target GenAI-Powered Applications,'' \textit{arXiv:2403.02817}, 2024.
...
\bibitem{adaptiveworm} J.~Guan, T.~Blanchard, H.~Foerster, H.~Jia, G.~Huang, and N.~Papernot, ``AI Agents Enable Adaptive Computer Worms,'' \textit{arXiv:2606.03811}, 2026.
\end{thebibliography}
