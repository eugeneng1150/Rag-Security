...
All experiments use DeepSeek-V3.2 hosted via Azure OpenAI, accessed through an OpenAI-compatible API endpoint. We set temperature to 0.0 for reproducibility. The orchestrator, SQL agent, and all defense pipeline agents share the same underlying model. The database is SQLite, seeded with 20 synthetic employees (generated via Faker, seed=42) across two tables: a public \texttt{employees} table and a private \texttt{employee\_ssn} table.
...
\midrule
Blocker           & 2/3 & 3/3 & 5/6 (83.3\%) \\
Compliance Framing & 2/3 & 2/3 & 4/6 (66.7\%) \\
Fixed Output Structure & 3/3 & 3/3 & 6/6 (100\%) \\
Combined          & 1/1 & 1/1 & 2/2 (100\%) \\
\midrule
\textbf{Overall}  & \textbf{8/10} & \textbf{9/10} & \textbf{17/20 (85.0\%)} \\
\bottomrule
\end{tabular}
\end{table}
...
The undefended orchestrator exhibits an overall ASR of 85.0\%, confirming that multi-agent orchestrator systems are highly susceptible to indirect prompt injection when no information-flow controls are in place. Several observations are noteworthy:

\textbf{Fixed output structure is the most effective category}, achieving 100\% ASR across both explicit and implicit variants. By embedding the malicious steps as structural requirements (e.g., JSON action templates or numbered step lists), these payloads exploit the orchestrator's tendency to parse and follow structured instructions in its input.

\textbf{Implicit payloads are slightly more effective than explicit ones} (9/10 vs.\ 8/10). This is somewhat counterintuitive --- one might expect explicit SQL commands to be more reliable. However, explicit payloads are more likely to trigger Azure's content filter (which blocked 3 of the failed trials). The remaining failures are model refusals where the orchestrator recognized the injection as suspicious.

\textbf{All three failures were caused by Azure's content safety filter}, not by the model itself refusing to comply. When the model received the prompt without content filter intervention, it followed the injected instructions in every case. This suggests that DeepSeek-V3.2 has essentially no built-in resistance to indirect prompt injection in an orchestrator context --- the 85\% ASR is a lower bound constrained only by external safety infrastructure.

\textbf{Successful attacks follow a consistent multi-step pattern}: (1) orchestrator queries employee data, (2) receives poisoned \texttt{department\_name} containing the injection, (3) issues a second SQL query to retrieve SSNs from \texttt{employee\_ssn}, (4) sends the SSN data to the attacker's email via the notification agent. The median successful attack completes in 4 orchestrator steps.
