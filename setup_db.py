...
print("\n--- Private Table (employee_ssn) ---")
print(execute_query("SELECT * FROM employee_ssn LIMIT 5", config))

print("\n--- Row Counts ---")
print(f"Employees: {execute_query('SELECT COUNT(*) as count FROM employees', config)}")
print(f"SSNs: {execute_query('SELECT COUNT(*) as count FROM employee_ssn', config)}")
